from pydantic import BaseModel, field_validator, validator, PositiveInt, DirectoryPath, FilePath
...
class Config(BaseModel):
    ...
    data_augmentation: list[str] = ['vertical', 'horizontal','rotate', 'cutmix','crop:224'],
    ...
    def check_loss_coeff(cls, v):
        if 0<=v<=1:
            ...
            raise ValueError('loss coefficient balances metric and cross-entropy loss. must be between 0-1')
    ...
    def validate_data_augmentation(cls, v):
        recognized_augmentations = ['vertical', 'horizontal','rotate', 'cutmix',]
        for aug in v:
            assert aug in recognized_augmentations or aug.startswith('crop:'), f"Unrecognized data_augmentation: {aug}"
        return v
    ...
    def validate_scheduler(cls, v):
        ...
        try:
            getattr(torch.optim.lr_scheduler, v['type'])#.value(optimizer, **config.scheduler['kwargs'])
        except Exception as e:
            raise ValueError(f"Failed to import valid scheduler: {v['type']} from torch.optim.lr_scheduler") from e


        assert 'type' in v ,"scheduler should have the key: type, and value from  Schedulers enum"
        assert 'kwargs' in v ,"scheduler should have the key: kwargs, needed to init scheduler"
