...
def make_transform_pipeline(resize_dim: Optional[int] = None, transform_list= ['crop:224','vertical','horizontal','rotate']):
    """
    ...
    if crop_dim:=[int(i.split(':')[-1]) for i in transform_list if i.startswith('crop')]:
        # apply image crop, example str format, crop:224
        crop_dim = crop_dim[0]
        _transforms.append(transforms.RandomCrop(crop_dim))  # Randomly crop the image 

        if resize_dim is not None:
            assert crop_dim < resize_dim, f"crop dimension {crop_dim} is not less than image after resizing {resize_dim}."
...
DEFAULT_TRANSFORM = make_transform_pipeline(None,['vertical','horizontal',])
...
class Rxrx1(Dataset):
    def __init__(self,
        ...
        meta['sirna']=meta['sirna'].astype('category') # numeric codes are assigned here
        meta['sirna_codes'] = meta['sirna'].cat.codes.astype('int64')
        meta['cell_type'] = meta['cell_type'].astype('category')
        meta['cell_type_codes'] = meta['cell_type'].cat.codes.astype('int64') # int64 needed for cross entropy loss
