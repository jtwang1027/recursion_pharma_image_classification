...
def load_model(model_type = 'densenet121', num_categories:int = 15, cell_embedding_dim: int = 12, image_size: int = 224):
    if 'densenet' in model_type:
        ...
        # ViT
        model =CustomVit(image_size = image_size,num_classes = num_categories, dropout = 0.2)
...
def train(config:Config):
    ...
    train_dataset = Rxrx1(
        ...
        data_transform = make_transform_pipeline(resize_dim=config.resize_img_dim, transform_list=config.data_augmentation))
    ...
    test_dataloader = iter(DataLoader(
                                Rxrx1(
                                    ...
                                    split = 'test', 
                                    num_categories= num_categories,
                                    data_transform = make_transform_pipeline(resize_dim=config.resize_img_dim)), # for test set: just resize, don't transform
    ...
    model = load_model(config.model, num_categories=num_categories, image_size = config.resize_img_dim)
    ...
    if config.use_scheduler:
        scheduler = getattr(torch.optim.lr_scheduler, config.scheduler['type']).value(optimizer, **config.scheduler['kwargs'])
    ...
    for epoch in range(num_epochs):
        ...
        for batch_num, (x, cell_type, labels) in tqdm(enumerate(train_dataloader)):
            x = x.to(device)
            # cell_type = cell_type.to(torch.int).to(device) # revert back to int after collate_fn converts this to float
            ...
            if config.use_cutmix:
                _metric_loss = metric_loss(pred_embedding_metric, labels) # when using cutmix, OHE not needed        
            else:
                _metric_loss = metric_loss(pred_embedding_metric, F.one_hot(labels, num_classes = config.num_categories))
            ...
            if batch_num % 50 ==0:
                ...
                test_x, test_cell_type, test_labels = test_x.to(device), test_cell_type.to(device), test_labels.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_path', type=str,  default='example_config.yaml',help="local path to config.yaml")#, required=True)
