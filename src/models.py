...
# from torchvision.models import vit_b_16, vit_b_32, vit_l_16, ViT_B_16_Weights, ViT_B_32_Weights
...
class CustomVit(nn.Module):
    def __init__(self, backbone = 'vit_b_16', 
        ...
        assert image_size % self.patch_size == 0, f"Image size({image_size}) and patch size must be evenly divisible ({self.patch_size})"
        
        self.num_classes = num_classes
        self.class_token = nn.Parameter(torch.zeros(1, 1, self.hidden_dim))

        self.conv_proj = nn.Conv2d(6,self.hidden_dim, kernel_size=self.patch_size, stride=self.patch_size)
        ...
        self.arc_margin_product = ArcMarginProduct(self.hidden_dim+cell_embedding_dim, self.num_classes)

    def forward(self, x, s):
        n, c, h, w = x.shape

        x = self.conv_proj(x) # after conv: batches, hidden_dim, num_patches, num_patches
        x = x.flatten(-2,-1) # flatten patches: batches, hidden_dim, num_patches**2
        x = x.permute(0,2,1) # attention needs: batches, flattened patches, patches_embedding (aka hidden_dim)
        x = torch.cat([self.class_token.expand(n, -1, -1), x], dim=1)
        
        x = self.encoder(x)[:,0]
        if self.cell_embedding_dim > 0:
            x = torch.cat([x, 
                            self.cell_embedding(s).squeeze(1)
                            ], dim=1)  
...
class CustomDensenet(nn.Module):
    def __init__(self, num_classes: int, backbone = 'densenet121',  embedding_size: int = 512, cell_embedding_dim: int =  12, ):
        # cell type expected to have 4 classes
        super().__init__()
